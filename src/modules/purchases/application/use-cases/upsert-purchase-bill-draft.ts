...
import type { PurchaseBillFormInput } from "@/modules/purchases/dto/purchase-bill.dto";
import { purchaseBillRepository } from "@/modules/purchases/infrastructure/repositories/purchase-bill.repository";

type UpsertPurchaseBillDraftInput = {
  organizationId: string;
  actorUserId: string;
  correlationId: string;
  data: PurchaseBillFormInput;
};
...
export async function upsertPurchaseBillDraft(input: UpsertPurchaseBillDraftInput) {
  return prisma.$transaction(async (tx) => {
    const organization = await accountingCoreRepository.getOrganizationContext(input.organizationId);
    const supplier = await purchaseBillRepository.findSupplierById(
    ...
    if (!supplier) {
      throw new NotFoundError("No encontramos el proveedor seleccionado.");
    }
    ...
    const items = await purchaseBillRepository.findItemsByIds(input.organizationId, itemIds, tx);
    const itemDefaultAccountIds = items
      .map((item) => item.defaultLedgerAccountId)
      .filter(Boolean) as string[];

    const [accounts, taxes] = await Promise.all([
      purchaseBillRepository.findAccountsByIds(
        input.organizationId,
        [...new Set([...explicitAccountIds, ...itemDefaultAccountIds])],
        tx,
      ),
      purchaseBillRepository.findTaxesByIds(input.organizationId, taxIds, tx),
    ]);
    ...
    const record = await purchaseBillRepository.saveDraft(
      {
