...
import { savePurchaseBillDraftAction } from "@/modules/purchases/application/commands/purchase-bill.commands";
import type {
...
type PurchaseBillFormDialogProps = {
  organizationSlug: string;
  dependencies: PurchaseBillFormDependenciesDto;
  bill?: PurchaseBillEditorDto;
};
...
function toOptionalDateInput(value?: string | null) {
  return value ? value.slice(0, 10) : "";
}

function getDefaultValues(bill?: PurchaseBillEditorDto): PurchaseBillFormValues {
  return {
    id: bill?.id,
    version: bill?.version,
    supplierId: bill?.supplierId ?? "",
    issueDate: toRequiredDateInput(bill?.issueDateIso),
    dueDate: toOptionalDateInput(bill?.dueDateIso),
    notes: bill?.notes ?? "",
    lines:
      bill?.lines.map((line) => ({
        itemId: line.itemId ?? "",
...
export function PurchaseBillFormDialog({
  organizationSlug,
  dependencies,
  bill,
}: PurchaseBillFormDialogProps) {
  ...
  const form = useForm<PurchaseBillFormValues>({
    resolver: zodResolver(purchaseBillFormSchema) as never,
  ...
  useEffect(() => {
    form.reset(getDefaultValues(bill));
  }, [bill, form, open]);
  ...
  const onSubmit = form.handleSubmit((values) => {
    ...
    startTransition(async () => {
      const result = await savePurchaseBillDraftAction(
        organizationSlug,
        values as unknown as PurchaseBillFormInput,
      );
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[92vh] max-w-6xl overflow-y-auto rounded-[28px] border border-emerald-950/10 bg-white p-0">
        <DialogHeader className="border-b border-emerald-950/5 px-6 py-5">
          <DialogTitle>{bill ? "Editar factura de compra" : "Nueva factura de compra"}</DialogTitle>
          <DialogDescription>
            El documento se guarda en borrador y toma numeracion oficial solo al publicarse.
          </DialogDescription>
        </DialogHeader>
        ...
        <form className="space-y-6 px-6 py-6" onSubmit={onSubmit}>
          <div className="grid gap-4 md:grid-cols-3">
            <div className="space-y-2">
              <Label htmlFor={`supplier-${bill?.id ?? "new"}`}>Proveedor</Label>
              <select
                id={`supplier-${bill?.id ?? "new"}`}
              ...
              >
                ...
                {dependencies.suppliers.map((supplier) => (
              ...
              {form.formState.errors.supplierId ? (
                <p className="text-sm text-rose-600">{form.formState.errors.supplierId.message}</p>
              ) : null}
            </div>

            <div className="space-y-2">
              <Label htmlFor={`issue-date-${bill?.id ?? "new"}`}>Fecha del documento</Label>
              <div className="relative">
                <CalendarDays className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" />
                <Input
                  id={`issue-date-${bill?.id ?? "new"}`}
                  type="date"
                  className="pl-10"
              ...
              {form.formState.errors.issueDate ? (
                <p className="text-sm text-rose-600">{form.formState.errors.issueDate.message}</p>
              ) : null}
            </div>

            <div className="space-y-2">
              <Label htmlFor={`due-date-${bill?.id ?? "new"}`}>Vencimiento</Label>
              <Input
                id={`due-date-${bill?.id ?? "new"}`}
                type="date"
          ...
          <div className="space-y-2">
            <Label htmlFor={`notes-${bill?.id ?? "new"}`}>Notas</Label>
            <Textarea
              id={`notes-${bill?.id ?? "new"}`}
              rows={3}
              placeholder="Observaciones operativas o de soporte del proveedor."
              {...form.register("notes")}
            />
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        placeholder="Servicio, compra o concepto"
          ...
          <div className="flex flex-col-reverse gap-3 border-t border-emerald-950/5 pt-5 sm:flex-row sm:justify-end">
            ...
            <Button type="submit" disabled={isPending} className="bg-emerald-700 text-white hover:bg-emerald-800">
              ...
              {bill ? "Guardar borrador" : "Crear borrador"}
            </Button>
          </div>
        </form>
      </DialogContent>
